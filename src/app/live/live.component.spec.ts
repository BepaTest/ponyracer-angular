...
describe('LiveComponent', () => {
  ...
  it('should not subscribe to the live observable if the race is FINISHED', () => {
    ...
    expect(liveComponent.positionSubscription).withContext('positionSubscription should be null').toBeNull();
  });
  ...
  it('should tidy things up when the race is over', () => {
    ...
    expect(liveComponent.winners).withContext('The winners should be empty until the race is over').toEqual([]);
    expect(liveComponent.betWon).withContext('The bet status should be null until the race is over').toBeNull();
    ...
    expect(liveComponent.winners.map(pony => pony.id))
      .withContext('The winners should contain all the ponies that won the race')
      .toEqual([1, 2]);
    expect(liveComponent.betWon).withContext('The bet status should be true if the player won the bet').toBe(true);
  });
  ...
  it('should buffer clicks over a second and call the boost method', fakeAsync(() => {
    ...
    fakeActivatedRoute.snapshot.data = { race };
    fakeRaceService.boost.and.returnValue(of(undefined));
    fakeRaceService.live.and.returnValue(EMPTY);
